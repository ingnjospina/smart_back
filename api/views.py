...
from .models import Alertas, Analisisaceitefisicoquimico, Analisisgasesdisueltos, MedicionesTransformadores, Transformadores
from .serializers import AlertasSerializer, AnalisisAceiteFisicoQuimicoSerializer, AnalisisGasesDisueltosSerializer, LoginSerializer, TransformadoresSerializer, UsuarioSerializer, MedicionesTransformadoresSerializer
    ...
    def post(self, request, *args, **kwargs):
        ...
        if serializer.is_valid():
            try:
                ...
                ruta_archivo = os.path.join(ruta_carpeta, f"medición_{serializer.data["idmediciones_transformadores"]}.json")
...
class MedicionesOne(APIView):
    ...
    def get(self, request, pk, *args, **kwargs):
        ...
        return Response({'gasesDisueltos': gasesSerializer.data, 'aceiteFisicoQuimico': aceitesSerializer.data}, status=status.HTTP_200_OK)
...
class MedicionesUpdateView(APIView):
    ...
    def put(self, request, pk, *args, **kwargs):
        try:
            ...
            gases = Analisisgasesdisueltos.objects.get(mediciones_transformadores_idmediciones_transformadores=pk)
            fisicoQuimico = Analisisaceitefisicoquimico.objects.get(mediciones_transformadores_idmediciones_transformadores=pk)
            ...
            return Response({"message": "El analisis de gases especificada no existe."}, status=status.HTTP_404_NOT_FOUND)
        except Analisisaceitefisicoquimico.DoesNotExist:
            return Response({"message": "El analisis de fisico quimico especificada no existe."}, status=status.HTTP_404_NOT_FOUND)

        try:
            serializer = MedicionesTransformadoresSerializer(medicion, data=request.data, partial=False)
            serializerGases = AnalisisGasesDisueltosSerializer(gases, data=request.data.get('analisis_gases_disueltos'), partial=False)
            serializerFisicoQuimico =AnalisisAceiteFisicoQuimicoSerializer(fisicoQuimico, data=request.data.get('analisis_aceite_fisico_quimico'), partial=False)
...
class AlertasListView(APIView):
    ...
    def get(self, request, *args, **kwargs):
        ...
        for medicion in medicion_temp: 
            alertas_queryset = Alertas.objects.filter(mediciones_transformadores=medicion['idmediciones_transformadores'])
...
class TranformadoresListView(APIView):
    ...
    def get(self, request, *args, **kwargs):
        queryset = Transformadores.objects.all()
